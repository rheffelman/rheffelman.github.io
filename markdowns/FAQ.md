## 1. "My algorithm doesn't consider x, so it cannot discriminate according to it."
...
**\#1.** Generate a dataset of numbers [[$a$, $b$], [$a$, $b$], ...] such that a and b are random natural numbers not divisible by 5.

**\#2.** Define and train a neural net such that, when given $a$ and $b$ it tries to predict $a$ XOR $b$. [(Bitwise XOR)](https://en.wikipedia.org/wiki/Bitwise_operation)
...
---
## 2."The data is anonymized, so everyone's right to confidentiality has been preserved."
This isn't necessarily true. The best example that I've seen of this is an [animal classification dataset](https://www.kaggle.com/datasets/uciml/zoo-animal-classification/data) that my professor showed me. Here are what the first 5 rows look like:
...
As you can see this data is anonymized, there is nothing that directly identifies which row is which animal, only the features of each animal. However, if you are clever you are able to roughly deduce the identity of each animal. Take this row for example:
...
```
And in a similar manner to the starfish, we deduce from var1:var5 that a certain id is you, the reader? Would you say your right to confidentiality has been preserved?

---

## 3. "Maybe AI systems are imperfect, but so are humans.  AI reduces bias compared to human decision-making."
...
- Certain medical applications: Certain AI systems in the medical field allow doctors and surgeons to do things they otherwise wouldn't be unable to do, such as see into peoples brains during surgery, and detect cancer extremely early. The end result is lives being saved.
